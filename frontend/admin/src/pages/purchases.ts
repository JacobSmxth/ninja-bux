...
export async function renderPurchases() {
  ...
  document.querySelectorAll(".tab").forEach((tab) => {
    tab.addEventListener("click", () => {
      currentStatus = (tab.getAttribute("data-status") || "") as
        | PurchaseStatus
        | "";
      document
...
function renderPurchasesTable() {
  ...
  tbody.querySelectorAll('[data-action="fulfill"]').forEach((btn) => {
    btn.addEventListener("click", () =>
      fulfillPurchase(Number(btn.getAttribute("data-id"))),
    );
  });

  tbody.querySelectorAll('[data-action="cancel"]').forEach((btn) => {
    btn.addEventListener("click", () =>
      cancelPurchase(Number(btn.getAttribute("data-id"))),
    );
  });
}
...
async function fulfillPurchase(purchaseId: number) {
  ...
  const response = await put(
    `/api/facilities/${facilityId}/purchases/${purchaseId}/fulfill`,
  );
...
async function cancelPurchase(purchaseId: number) {
  ...
  const facilityId = getCurrentFacilityId();
  const response = await put(
    `/api/facilities/${facilityId}/purchases/${purchaseId}/cancel`,
  );
