...
export async function renderShop() {
  ...
  document
    ...
    ?.addEventListener("click", () => showItemModal());

  // Fetch items
  const response = await get<ShopListResponse>(
...
function renderShopTable() {
  ...
  if (shopItems.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="empty">No shop items. Add one to get started!</td></tr>';
    return;
  }

  tbody.innerHTML = shopItems
    .map(
      (item) => `
    <tr>
      <td>${item.name}</td>
...
async function saveItem(itemId?: number) {
  ...
  const description = (
    document.getElementById("item-description") as HTMLInputElement
  ).value;
  const price = Number(
    (document.getElementById("item-price") as HTMLInputElement).value,
  );
  const isAvailable = (
    document.getElementById("item-available") as HTMLInputElement
  ).checked;
  ...
  let response;
  if (itemId) {
    response = await put<ShopItem>(
      `/api/facilities/${facilityId}/shop/${itemId}`,
      payload,
    );
  } else {
    response = await post<ShopItem>(
      `/api/facilities/${facilityId}/shop`,
      payload,
    );
  }
...
async function toggleItem(itemId: number) {
  ...
  const facilityId = getCurrentFacilityId();
  const response = await put<ShopItem>(
    `/api/facilities/${facilityId}/shop/${itemId}`,
    {
      name: item.name,
