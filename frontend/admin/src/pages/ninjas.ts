...
export async function renderNinjas() {
  ...
  // Fetch ninjas
  const response = await get<NinjaListResponse>(`/api/facilities/${facilityId}/ninjas?size=500`);
  allNinjas = response.data?.ninjas || [];

  renderNinjasTable();

  // Setup filters
  document.getElementById('search-input')?.addEventListener('input', renderNinjasTable);
  document.getElementById('belt-filter')?.addEventListener('change', renderNinjasTable);
}
...
function renderNinjasTable() {
  ...
  const beltFilter = (document.getElementById('belt-filter') as HTMLSelectElement)?.value || '';
  ...
  if (filtered.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" class="empty">No ninjas found</td></tr>';
    return;
  }

  tbody.innerHTML = filtered.map(ninja => `
    <tr>
...
async function showLedgerModal(studentId: string) {
  const facilityId = getCurrentFacilityId();
  const response = await get<LedgerResponse>(`/api/facilities/${facilityId}/ninjas/${studentId}/ledger?limit=20`);
