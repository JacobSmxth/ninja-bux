...
function renderAdminsTable() {
  ...
  if (allAdmins.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" class="empty">No admins found</td></tr>';
    return;
  }
...
function showAdminModal(admin?: AdminUser) {
  ...
  showModal({
    ...
    onConfirm: async () => {
      ...
      const superAdmin = (document.getElementById('admin-super') as HTMLInputElement).checked;
      const facilityCheckboxes = document.querySelectorAll<HTMLInputElement>('input[name="facilities"]:checked');
      ...
      showAlert(`Admin ${isEdit ? 'updated' : 'created'} successfully`, 'success');
  ...
  const superCheckbox = document.getElementById('admin-super') as HTMLInputElement;
  const facilitiesGroup = document.getElementById('facilities-group') as HTMLDivElement;
