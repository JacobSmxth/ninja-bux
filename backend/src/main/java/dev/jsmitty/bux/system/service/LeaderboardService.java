...
public class LeaderboardService {
    ...
    public LeaderboardResponse getTopEarners(UUID facilityId, String period, int limit) {
        ...
        List<Object[]> results =
                ledgerTxnRepository.findTopEarners(
                        facilityId, TxnType.ADJUSTMENT, since, PageRequest.of(0, limit));
    ...
    private List<LeaderboardEntry> buildLeaderboardEntries(
        ...
        for (Object[] row : results) {
            ...
            if (isEarned) {
                entries.add(
                        LeaderboardEntry.earned(rank, studentId, name, total.intValue(), balance));
            } else {
                entries.add(
                        LeaderboardEntry.spent(rank, studentId, name, total.intValue(), balance));
            }
            rank++;
        }
